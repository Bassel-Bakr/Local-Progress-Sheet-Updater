...
class Gui:
    def __init__(self, config: Config):
        ...
        if self.config.open_config:
            ...
            self.run_mode_options = ["once", "watchdog", "interval"]
    ...
    def new_range(self):
        ...
        # Labels
        header_label = Label(
            self.range_frame_list[-1],
            text="Range Number: " + str(len(self.range_frame_list)),
        )
        ...
        self.range_frame_list[-1].grid(
            row=(len(self.range_frame_list) - 1) // 3,
            column=(len(self.range_frame_list) - 1) % 3,
        )
    ...
    def finished(self):
        ...
        self.config.scenario_name_ranges = [
            entry.get() for entry in self.name_ranges_entries
        ]
        self.config.highscore_ranges = [
            entry.get() for entry in self.highscore_ranges_entries
        ]
        self.config.average_ranges = [
            entry.get() for entry in self.average_ranges_entries
        ]
    ...
    def main(self):
        if self.config.open_config:
            ...
            sheet_id_entry_kovaaks = Entry(
                sheet_id_frame_kovaaks, textvariable=self.sheet_id_kovaaks
            )
            pre_sheet_id_label_kovaaks = Label(
                sheet_id_frame_kovaaks, text="Progress Sheet ID: "
            )
            ...
            sheet_id_entry_aimlab = Entry(
                sheet_id_frame_aimlab, textvariable=self.sheet_id_aimlab
            )
            pre_sheet_id_label_aimlab = Label(
                sheet_id_frame_aimlab, text="Progress Sheet ID: "
            )
            ...
            # Open config
            open_config_box = Checkbutton(
                advanced_frame, text="Open config", variable=self.open_config
            )
            open_config_box.grid(
                row="0", column="0", padx=advanced_padding_x, pady=advanced_padding_y
            )
            # Polling interval
            polling_interval_frame = Frame(advanced_frame)
            polling_interval_label = Label(
                polling_interval_frame, text="Polling Interval"
            )
            polling_interval_entry = Entry(
                polling_interval_frame, textvariable=self.polling_interval, width=5
            )
            polling_interval_entry.pack(side="left")
            polling_interval_label.pack(side="left")
            polling_interval_frame.grid(
                row="0", column="1", padx=advanced_padding_x, pady=advanced_padding_y
            )
            # Run mode
            run_mode_frame = Frame(advanced_frame)
            run_mode_label = Label(run_mode_frame, text="Run Mode")
            run_mode_dropdown = OptionMenu(
                run_mode_frame, self.run_mode, *self.run_mode_options
            )
            run_mode_label.pack(side="left")
            run_mode_dropdown.pack(side="left")
            run_mode_frame.grid(
                row="0", column="2", padx=advanced_padding_x, pady=advanced_padding_y
            )

            # Runs to average
            runs_to_average_frame = Frame(advanced_frame)
            runs_to_average_entry = Entry(
                runs_to_average_frame, textvariable=self.runs_to_average, width=3
            )
            runs_to_average_label = Label(
                runs_to_average_frame, text="Number of runs to average"
            )
            runs_to_average_entry.pack(side="left")
            runs_to_average_label.pack(side="left")
            # Calculate Averages (Kovaaks only atm)
            calculate_averages_box = Checkbutton(
                runs_to_average_frame,
                variable=self.calculate_averages,
                text="Calculate Averages",
            )
            calculate_averages_box.pack(side="top")
            runs_to_average_frame.grid(
                row="0", column="3", padx=advanced_padding_x, pady=advanced_padding_y
            )
            ...
            while i < len(self.config.scenario_name_ranges):
                ...
                # Labels
                header_label = Label(
                    self.range_frame_list[i], text="Range Number: " + str(i + 1)
                )
                ...
                # Add new entries
                self.name_ranges_entries.append(
                    Entry(self.range_frame_list[i], width=32)
                )
                self.highscore_ranges_entries.append(
                    Entry(self.range_frame_list[i], width=32)
                )
                self.average_ranges_entries.append(
                    Entry(self.range_frame_list[i], width=32)
                )
                # Set value of entries
                self.name_ranges_entries[i].insert(
                    0, self.config.scenario_name_ranges[i]
                )
                self.highscore_ranges_entries[i].insert(
                    0, self.config.highscore_ranges[i]
                )
            ...
            new_range_button = Button(
                range_button_frame, command=self.new_range, text="Add Range"
            )
            new_range_button.grid(row="0", column="0", columnspan="2")
            del_range_button = Button(
                range_button_frame, command=self.delete_range, text="Remove Range"
            )
            ...
            finished_button = Button(
                finished_frame, command=self.finished, text="Finish"
            )
